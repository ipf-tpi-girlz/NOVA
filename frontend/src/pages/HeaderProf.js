...
export const HeaderProf = () => {
    ...
    const $columnContainer = createElementWithClasses('div', ["flex",
        "flex-col",
        "lg:flex-row",
        "gap-8"]);

    const $leftColumn = createElementWithClasses('div', ['flex-1', 'bg-base-200', "border", "border-gray-300", 'rounded-3xl', 'shadow-lg', 'p-6', 'transition-all', 'duration-300', 'hover:shadow-xl']);  // Aplicar scroll-invisible aquí
    ...
    const $rightColumn = createElementWithClasses('div', ['flex-1',
        'bg-base-200',
        'backdrop-filter',
        'backdrop-blur-lg',
        'border',
        ...
        'transition-all',
        'duration-500',
        'hover:shadow-2xl']);  // Aplicar scroll-invisible aquí
    ...
    const communitiesContainer = createElementWithClasses('div', ['grid',
        'grid-cols-1',
        'md:grid-cols-2',
        'lg:grid-cols-3',
        'gap-6',
...
const loadComunities = async () => {
    try {
        ...
        if (!communities || communities.length === 0) {
            console.log("¡Aún no has creado ninguna comunidad! 🌱");
...
const NoContent = () => {
    ...
    description.textContent = "Aún no ha sido creado contenido.";
...
const createCommunityCard = (community) => {
    ...
    const contentWrapper = document.createElement('div');
    contentWrapper.classList.add('flex-grow', 'p-6',
        'space-y-3',
        'flex-grow',
        'relative',
        'z-10');
    ...
    title.innerHTML = `${community.nombre || 'Comunidad'} <span class="text-sm text-purple-500">✨</span>`;
    ...
    button.innerHTML = `<span>Ver mas</span>`;
    ...
    contentWrapper.append(title, desc);
