...
export const ContactsPage = async () => {
  ...
  interfazProfesionales.className =
    "flex flex-grow bg-base-200 dark:bg-base-100  max-w-6xl p-5 rounded-md m-4 shadow-2xl";
  ...
  title.className =
    "break-all text-center font-bold font-serif text-2xl mb-2 m4";

  const desc = document.createElement("p");
  desc.className = " font-serif text-sm text-center";
  desc.textContent =
    "Si desea obtener mas informacion de un profesional, haga click sobre el.";

  containertitle.appendChild(title);
  containertitle.appendChild(desc);
  const profesional = document.createElement("div");
  profesional.className =
    "flex flex-col justify-content  gap-2 overflow-auto max-h-[64vh]";
  ...
  try {
    ...
    profiles.forEach((profile) => {
      const {
        nombre,
        img,
        perfil: { descripcion, especialidad },
        id,
      } = profile;
...
const inspirationContainer = () => {
  ...
  container.className =
    "relative flex flex-col items-center justify-center w-full max-w-xs bg-base-100 p-5 rounded-md m-10 overflow-hidden shadow-2xl";

  // Añadir un fondo de imagen al contenedor con un filtro de brillo
  container.style.backgroundImage =
    "url('https://i.pinimg.com/736x/28/8f/82/288f82985709fed6986e02f7920c2877.jpg')";
  ...
  textContainer.className =
    "absolute inset-0 flex flex-col items-center justify-center p-4 opacity-0 transform translate-y-10 transition-all duration-500";
  ...
  description.textContent =
    '"La ayuda está a un paso de ti. Tu seguridad y bienestar son lo más importante. Da el primer paso hacia una vida libre de miedo y llena de paz."';
