...
export const Contactanos = () => {
  ...
  modalBox.className =
    "modal-box shadow-lg text-center bg-white bg-cover text-base-content";
  ...
  formWrapper.className =
    "w-full max-w-xl rounded-2xl p-8 space-y-6 bg-white/90 backdrop-blur-lg"; // Fondo más claro y con desenfoque de fondo
  ...
  $title.className = "text-3xl font-bold text-gray-600 tracking-tight";

  const $subtitle = document.createElement("h2");
  $subtitle.textContent =
    "Tu opinión nos ayuda a mejorar para dar una mejor experiencia";
  ...
  fields.forEach((field) => {
    ...
    if (field.type === "textarea") {
      ...
      $textarea.rows = 4;
      $textarea.className =
        "w-full pl-10 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 ease-in-out resize-none bg-white/70 placeholder-gray-400";
      ...
      $input.className =
        "w-full pl-10 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 ease-in-out bg-white/70 placeholder-gray-400";
  ...
  $form.addEventListener("submit", (event) => {
    ...
    emailjs.sendForm(serviceID, templateID, $form).then(
      ...
      (err) => {
        ...
        showNotification(
          "error",
          "Error al enviar el mensaje: " + JSON.stringify(err)
        );
      }
    );
  });
