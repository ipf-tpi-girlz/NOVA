...
export const updateUser = async (req, res) => {
    const { id } = req.params;
    const { role, nombre, mail, departamento, localidad, contrasenia, nro_telefono, nro_matricula, razon_social, direccion } = req.body;
    ...
    try {
        ...
        if (role === 'institucion') {
            const institucion = await Institucion.findOne({ where: { usuario_id: usuario.id } });
            if (institucion) {
