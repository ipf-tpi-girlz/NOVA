...
export const getSubforos = async (req, res) => {
  try {
    ...
    if (subforos.length === 0)
      return res
        .status(404)
        .json({ message: "No existen subforos en el sistema" });
...
export const updateSubForo = async (req, res) => {
  ...
  try {
    ...
    if (id_user !== moderador)
      return res
        .status(400)
        .json({ message: "Solo el moderador puede realizar esta operación" });
    //*actualizar subforo
