...
export const getPostsComunity = async (req, res) => {
  ...
  try {
    ...
    if (posts.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "No se encontraron posts en esta comunidad" });
    }
...
export const updatePostComunity = async (req, res) => {
  ...
  try {
    ...
    if (post.usuario_id !== user.id) {
      ...
      return res
        .status(403)
        .json({ message: "Solo el dueño de la publicación puede modificarla" });
    }
    await post.update({
      titulo,
      contenido,
...
export const deletePostComunity = async (req, res) => {
  ...
  try {
    ...
    if (post.usuario_id !== user.id) {
      ...
      return res
        .status(403)
        .json({ message: "Solo el dueño de la publicación puede eliminarla" });
    }
    await post.destroy();
